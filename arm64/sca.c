...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "vtrap_sca", _hoc_vtrap,
 0, 0
};
#define Exp Exp_sca
#define vtrap vtrap_sca
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 sca /Users/melissafasol/Desktop/Dendrite Modelling/sca.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
static int _reset;
static char *modelname = "High-Voltage Activated (HVA) Ca2+ current";
...
double vtrap ( _threadargsprotocomma_ double _lx , double _ly ) {
 ...
 if ( fabs ( _lx / _ly ) < 1e-6 ) {
     _lvtrap = 1.0 * _ly * ( 1.0 - _lx / _ly / 2.0 ) ;
     }
   else {
     _lvtrap = 1.0 * _lx / ( Exp ( _threadargscomma_ _lx / _ly ) - 1.0 ) ;
     }
...
static int  rates ( _threadargsprotocomma_ double _lv ) {
 ...
 _lalpha = 0.055 * vtrap ( _threadargscomma_ - ( _lv + 35.0 ) , 1.0 ) ;
   _lbeta = 0.94 * exp ( - ( _lv + 75.0 ) / 17.0 ) ;
   mtau = 1.0 / ( _lalpha + _lbeta ) ;
   minf = _lalpha / ( _lalpha + _lbeta ) ;
   _lalpha = 0.000152 * exp ( - ( _lv + 13.0 ) / 50.0 ) ;
   _lbeta = 0.00217 / ( exp ( - ( _lv + 15.0 ) / 28.0 ) + 1.0 ) ;
...
static double _nrn_current(double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt, double _v){double _current=0.;v=_v;{ {
   ...
   ica = tadj * gcabar * pow ( m , 2.0 ) * h * ( v - eca ) ;
   }
 _current += ica;
...
#if NMODL_TEXT
static const char* nmodl_filename = "/Users/melissafasol/Desktop/Dendrite Modelling/sca.mod";
static const char* nmodl_file_text = 
